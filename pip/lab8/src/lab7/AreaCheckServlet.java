...
public class AreaCheckServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Mark mark = new Mark(xVal, yVal, rVal);

        mark.status = checkMark(mark, rVal);

        HttpSession session = request.getSession();
        Vector<Mark> points = (Vector<Mark>)session.getAttribute("Points");
        if(points == null)
            points = new Vector<Mark>();
        points.addElement(mark);
        session.setAttribute("Points", points);

        PrintWriter out = response.getWriter();
        try {
            timeout = System.currentTimeMillis() - timeout;
            out.println(getPage(mark, rVal, mark.status, timeout));
        } finally {
            out.close();
        }
    }
    ...
    private String getPage(Mark mark, Float r, Boolean status, long timeout) {
        ...
        return "<!DOCTYPE html>\n" +
                ...
                "<form id=\"input_form\" action=\"/lab8_Web/lab8\" method=\"GET\">\n" +
